...
// ─────────────────────────────────────────────────────────────────────────────
// AJAX — cek apakah data sudah diimport
// ─────────────────────────────────────────────────────────────────────────────
if (($_GET['action'] ?? '') === 'check_data') {
    try {
        $db  = makeDb();
        $txn = (int)$db->query("SELECT COUNT(*) FROM transactions")->fetchColumn();
        $mat = (int)$db->query("SELECT COUNT(*) FROM transaction_materials")->fetchColumn();
        $ok  = $txn > 0;
        jsonOut(['ok'=>$ok,'txn'=>$txn,'mat'=>$mat,
                 'message'=> $ok ? "$txn transaksi, $mat materials sudah ada di DB." : "Belum ada data. Jalankan Python script dulu."]);
    } catch(\Exception $e) { jsonOut(['ok'=>false,'message'=>$e->getMessage()]); }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// AJAX — setup photo migration tables + mapping dari DB
// ─────────────────────────────────────────────────────────────────────────────
if (($_GET['action'] ?? '') === 'setup_photos') {
    try {
        ...
        // Baca label_photos langsung dari transaction_materials di DB
        $rows = $db->query(
            "SELECT id, label_photos FROM transaction_materials
             WHERE label_photos IS NOT NULL AND label_photos <> ''"
        )->fetchAll();

        // Insert unique GDrive IDs
        $allIds = [];
        foreach ($rows as $r) {
            foreach (explode(',', $r['label_photos']) as $id) {
                $id = trim($id);
        ...
        // Insert mappings langsung dari tm.id — tidak perlu lookup uuid/mat
        $mapStmt = $db->prepare("INSERT INTO photo_migration_map(tm_id,gdrive_id,position) VALUES(:t,:g,:p)");
        $maps = 0;
        foreach ($rows as $r) {
            foreach (explode(',', $r['label_photos']) as $pos=>$id) {
                $id = trim($id); if (!isGdriveId($id)) continue;
                $mapStmt->execute([':t'=>(int)$r['id'],':g'=>$id,':p'=>$pos]); $maps++;
            }
        }
...
<div class="card-body">
  ...
  <div class="row g-2 align-items-end">
    ...
    <div class="col-auto mt-2"><button class="btn btn-sm btn-primary" onclick="act('test_db','log_db')">Test & Simpan</button></div>
  </div>
...
<div class="card-body">
  ...
  <p class="small text-muted mb-2">Import dilakukan via <strong>Python script</strong> langsung dari terminal — tidak butuh upload ke browser.</p>
  <div class="bg-dark rounded p-3 mb-3" style="font-family:monospace;font-size:.8rem;color:#00ff88;white-space:pre">
# Pastikan data_lama.xlsx ada di root folder project (sejajar folder active/)
# Jalankan dari terminal di folder project:

python python_bot/import_excel.py

# Estimasi: 2-5 menit untuk ~2.400 transaksi + 74.000 materials</div>
  <p class="small text-muted mb-2">Setelah script selesai, klik <strong>Cek Data</strong> untuk verifikasi.</p>
  <button class="btn btn-sm btn-outline-primary" onclick="checkData()">&#128269; Cek Data</button>
  <div id="log_import" class="log" style="height:40px">-</div>
</div>
</div>
...
async function act(action, logId){
    const logMap={test_db:'log_db',run_schema:'log_schema',seed_users:'log_users',setup_photos:'log_photos_setup'};
    const lid = logId || logMap[action] || 'log_'+action;
...
async function checkData(){
    lg('log_import','Mengecek data di DB...','#ffcc00');
    const d=await fetch('?action=check_data').then(r=>r.json()).catch(e=>({ok:false,message:e.message}));
    lg('log_import',d.message,d.ok?'#00ff88':'#ff4444');
