...
function dbShellQuery(string $sql): string {
    $pgPass = getenv('PRODADMIN_DB_PASS') ?: 'SASMU123';
    $dbName = getenv('PRODADMIN_DB_NAME') ?: 'prod_admin';
    $dbUser = getenv('PRODADMIN_DB_USER') ?: 'postgres';
    $dbHost = getenv('PRODADMIN_DB_HOST') ?: 'localhost';
    $dbPort = getenv('PRODADMIN_DB_PORT') ?: '5432';
    $psql = getenv('PRODADMIN_PSQL') ?: 'C:/Program Files/PostgreSQL/18/bin/psql.exe';

    $cmd = 'set "PGPASSWORD=' . $pgPass . '" && "' . $psql . '"'
...
// ── Verifikasi DB ─────────────────────────────────────────────────────────────
echo "\n=== Verifikasi DB (psql) ===\n";
$dbCheck = dbShellQuery("SELECT COUNT(*) FROM transactions WHERE uuid='$UUID'");
check('transactions: 1 row',  trim($dbCheck) === '1');

$matCheck = dbShellQuery("SELECT COUNT(*) FROM transaction_materials WHERE transaction_uuid='$UUID'");
check('transaction_materials: 2 rows', trim($matCheck) === '2');

$hourCheck = dbShellQuery("SELECT COUNT(*) FROM material_hourly_usage mhu JOIN transaction_materials tm ON tm.id=mhu.transaction_material_id WHERE tm.transaction_uuid='$UUID'");
check('material_hourly_usage: > 0 rows', (int)trim($hourCheck) > 0);

$outCheck = dbShellQuery("SELECT COUNT(*) FROM production_outputs WHERE transaction_uuid='$UUID'");
check('production_outputs: 1 row', trim($outCheck) === '1');

$repCheck = dbShellQuery("SELECT COUNT(*) FROM production_reports WHERE transaction_uuid='$UUID'");
...
$statCheck = dbShellQuery("SELECT status FROM transactions WHERE uuid='$UUID' AND status NOT IN ('HISTORY','SUPERSEDED') LIMIT 1");
...
check('success=true', $r['success'] === true);
$delCheck = dbShellQuery("SELECT COUNT(*) FROM transactions WHERE uuid='$UUID'");
